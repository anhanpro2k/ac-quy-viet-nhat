...
function get_taxonomy_level( $term_id, $taxonomy ) {
	$term  = get_term( $term_id, $taxonomy );
	$level = 1;
	while ( ! empty( $term->parent ) && $term->parent > 0 ) {
...
function mona_pagination_links_products( $wp_query = '', $paged = '' ) {
	if ( $wp_query == '' ) {
		global $wp_query;
	}
	$bignum = 999999999;
	...
	if ( is_tax( 'product-cat' ) ) {
		$permalink = get_term_link( get_queried_object()->term_id, get_queried_object()->taxonomy );
	} else {
		$permalink = get_the_permalink( MONA_WC_PRODUCTS );
	}
