...
<?php
$mona_home_section_cat2 = get_field( 'mona_home_section_cat2' );
?>

<?php
if ( content_exists( $mona_home_section_cat2 ) ) {
	?>
    <div class="home-prodbox mt4">
        <div class="container">
            <div class="home-prodbox-wr tabJS">
                <div class="head" data-aos="fade-up">
					<?php
					$cat2_category = $mona_home_section_cat2['cat2_category'];
					?>
					<?php
					if ( ! empty( $cat2_category ) ) {
						?>
                        <h2 class="head-tt">
							<?php echo get_term( $cat2_category )->name; ?>
                        </h2>
						<?php
					}
					?>
					<?php
					$cat2_brand = $mona_home_section_cat2['cat2_brand'];
					?>
					<?php
					if ( content_exists( $cat2_brand ) ) {
						?>
                        <div class="head-tabbtn scrollDeskJS">
							<?php
							foreach ( $cat2_brand as $key_brand => $item_brand ) {
								?>
								<?php
								if ( ! empty( $item_brand ) ) {
									?>
					...
					<?php
					$cat2_link = $mona_home_section_cat2['cat2_link'];
					?>
					<?php
					if ( ! empty( $cat2_link ) ) {
						?>
                        <a target="_blank" href="<?php echo esc_url( $cat2_link['url'] ); ?>" class="btn-seeallyel">
                            <span class="txt"><?php echo ! empty( $cat2_link['title'] ) ? $cat2_link['title'] : __( 'Xem tất cả', 'monamedia' );; ?></span>
                ...
                <div class="home-prodbox-panel" data-aos="fade-up" data-aos-delay="400">
					<?php
					if ( content_exists( $mona_home_section_cat2['cat2_brand'] ) ) {
						?>
						<?php
						foreach ( $mona_home_section_cat2['cat2_brand'] as $key_brand => $item_brand ) {
							if ( ! empty( $item_brand ) ) {
								?>
